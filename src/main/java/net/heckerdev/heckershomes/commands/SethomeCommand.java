        ...
        getServer().getScheduler().runTaskAsynchronously(HeckersHomes.getInstance(), () -> {
            if (!sender.hasPermission("heckershomes.command.sethome")) {
                ...
                if (args.length == 0) {
                    ...
                    if (StorageUtil.getHomeCount(player) >= StorageUtil.getMaxHomes(player).join()) {
                        ...
                        if (StorageUtil.setNewHome(player, args[0], playerLoc.getWorld().getName(), playerLoc.getX(), playerLoc.getY(), playerLoc.getZ(), playerLoc.getYaw(), playerLoc.getPitch()).join()) {
